...
Background:
	#panda系统中：创建供货商、设置供货商运费、同步商品到自营平台
		#创建供货商
			Given 创建两个特殊的供货商，就是专门针对商品池供货商
				...
				"""
		#设置供货商运费
			#供货商1和供货商2设置运费-满100包邮，否则收取运费10元
			When 给供货商添加运费配置
				"""
				[{
					"supplier_name": "供货商1",
	...
	#开放平台中：创建使用账号 ，激活，审批 准许使用API接口
		...
		Given 自营平台已获取jd订单
			...
			"""
Scenario:1 通过主订单ID提供订单详情API '待发货'
#商品1-1待发货，商品1-2待发货	
	When jd调用'订单详情'api
		"""
			{
...
#商品1-1已完成，商品1-2待发货	
	...
	Then jd获取'订单详情'api返回结果		
		...
		"""
Scenario:2 通过主订单ID提供订单详情API '已发货'
#商品1-1已发货，商品1-2已发货
	#Given 自营平台订单数据已同步到panda系统中
	Given pd登录panda系统
	When pd对订单进行发货
...
#商品1-1已完成，商品1-2已发货
	...
	Then jd获取'订单详情'api返回结果		
		...
		"""
Scenario:3 通过主订单ID提供订单详情API '已完成'
	#Given 自营平台订单数据已同步到panda系统中
	Given pd登录panda系统
	When pd完成订单
		"""
