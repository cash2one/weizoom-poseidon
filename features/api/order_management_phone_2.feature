...
Feature: 自营平台订单管理-手机端-两个供应商商品订单-支付宝+积分(一个不满足满额包邮，一个无运费)；退优惠券，积分(积分按照现在的比例)
"""
	一、后台订单列表
...
Scenario:1 ziying两个供应商商品订单-支付宝+积分-待支付
	#待支付订单
		#手机端订单列表
...
@refund @order @allOrder
Scenario:2 ziying两个供应商商品订单-支付宝+积分-待发货
	#待发货订单
		When bill访问zy1的webapp::apiserver
...
@refund @order @allOrder
Scenario:3 ziying两个供应商商品订单-支付宝+积分-已发货
	#已发货
		When bill访问zy1的webapp::apiserver
		When bill使用支付方式'支付宝'进行支付订单'002'于2016-01-03 10:00:00::apiserver
...
@refund @order @allOrder
Scenario:4 ziying两个供应商商品订单-支付宝+积分-已完成
	#已完成
		When bill访问zy1的webapp::apiserver
		When bill使用支付方式'支付宝'进行支付订单'002'于2016-01-03 10:00:00::apiserver
...
@refund @order @allOrder
Scenario:5 ziying两个供应商商品订单-支付宝+积分-退款中
	#退款中
		When bill访问zy1的webapp::apiserver
		When bill使用支付方式'支付宝'进行支付订单'002'于2016-01-03 10:00:00::apiserver
...
@refund @order @allOrder
Scenario:6 ziying两个供应商商品订单-支付宝+积分-退款完成
	#退款完成
		When bill访问zy1的webapp::apiserver
		When bill使用支付方式'支付宝'进行支付订单'002'于2016-01-03 10:00:00::apiserver
