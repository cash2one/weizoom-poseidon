...
class Command(BaseCommand):
	...
	def __touch_file(self, fname, times=None):
		with open(fname, 'a'):
	...
	def copy_app_template_to(self, app_dir, app_name):
		app_template_dir = os.path.join(settings.PROJECT_HOME, '../app/app_template')
	...
	def copy_app_react_template_to(self, react_dir):
		react_template_dir = os.path.join(settings.PROJECT_HOME, '../app/app_template/react')		
		if os.path.exists(react_dir):
			print '[react] remove old react dir: ', react_dir
			shutil.rmtree(react_dir)

		shutil.copytree(react_template_dir, react_dir)

	def generate_resource_files(self, app_dir, app):
		#收集resource
		resources = []
		for resource in app['resources']:
			resource['plural_name'] = 'activities'#self.__get_plural_name(resource['name']).lower()
			resource['capitalized_plural_name'] = resource['plural_name'].capitalize()
			...
			resource['file_name'] = 'activity'
			resource['second_nav'] = 'activities'
			resource['class_name'] = 'Activity'
			resource['lower_name'] = 'activity'
			...
			if resource['need_user_participant']:
				...
				participance_resource['enable_wepage'] = False
				participance_resource['is_main_entry'] = False
				...
				participances_resource['is_main_entry'] = False
			...
			mobile_resource['is_main_entry'] = False
			...
			status_resource['is_main_entry'] = False
			...
			collection_resource['is_main_entry'] = False
			...
			collection_resource['lower_name'] = 'activities'
			collection_resource['second_nav'] = 'activities'
			collection_resource['display_name'] = u'活动列表'
			collection_resource['item_class_name'] = collection_resource['class_name']
			collection_resource['file_name'] = 'activities'
			collection_resource['class_name'] = 'Activities'
		...
		#生成nav.py
		tmpl_file_path = os.path.join(app['name'], 'nav_template.py')
		dst_file_path = os.path.join(app_dir, 'nav.py')
		self.__render_template_file(tmpl_file_path, context, dst_file_path)

		#生成models.py
		tmpl_file_path = os.path.join(app['name'], 'mongo_models_template.py')
		dst_file_path = os.path.join(app_dir, 'mongo_models.py')
		self.__render_template_file(tmpl_file_path, context, dst_file_path)
	...
	def generate_react_files(self, app_dir, app):
		def __render_template_files_in_dir(dir, template_prefix, context):
			print '[react] render all react file under %s' % dir
			for f in os.listdir(dir):
				src_file_path = os.path.join(template_prefix, f)
				dst_file_path = os.path.join(dir, f)
				self.__render_template_file(src_file_path, context, dst_file_path)
		...
		for resource in app['resources']:
			...
			react_dir = os.path.join('static/component/app/app/', app['name'])
			self.copy_app_react_template_to(react_dir)

			context = Context({
				'uppercase_app_name': app['uppercase_app_name'],
				'app_name': app['name'],
				'app_display_name': app['display_name'],
				'resource': resource
			})
			activities_dir = os.path.join(react_dir, 'activities')
			template_prefix = '%s/activities' % app['name']
			__render_template_files_in_dir(activities_dir, template_prefix, context)
			activity_dir = os.path.join(react_dir, 'activity')
			template_prefix = '%s/activity' % app['name']
			__render_template_files_in_dir(activity_dir, template_prefix, context)

			#touch index.js, trigger webpack
			index_js_file = 'static/index.js'
			self.__touch_file(index_js_file)
